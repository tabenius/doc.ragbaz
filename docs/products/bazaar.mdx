...
**Bazaar** covers anything in the RAGBAZ workspace that relates to packaging, selling, documenting, and deploying components:

- **Docker Compose–based components** — services wired via `docker-compose.yml`, each with its own Dockerfile, health checks, and inter-service dependencies
- **Components depending on ragbaz modules/repos/subsystems** — any service that references another RAGBAZ project, design system, or shared library
- **Self-host/deploy aspects** — deployment pipelines (Docker, systemd, compose), environment configuration, DNS setup, certificate management
- **Price + included cards list** — tiered pricing (free / paid / subscription), feature breakdown per tier, what's included in each card
- **Marketplace/shop** — listing CRUD, cart, checkout, specimen viewers, order management
- **Documentation & technical documentation** — per-component docs, API references, architecture specs, deployment guides, versioned manuals

A listing in the bazaar captures all of these facets. Every RAGBAZ product, experiment, and infrastructure component is a potential bazaar listing.

### BAZaar (the storefront)

BAZaar is the marketplace storefront that surfaces bazaar listings. Every listing is:
...
See [MATCHES](./matches) for the flagship simulation product used as the specimen example above, [Articulate](./articulate/overview) for the WordPress commerce platform BAZaar deploys to, and [Products Overview](./overview) for the full portfolio.
