...
The Konsonans AI Governance Platform — a policy-as-code control plane for EU AI Act compliance. A `glither.governance` policy dialect compiled to a WASM component enforces risk-tiered allow/hold/deny on every agent tool-call; a PostgreSQL append-only audit chain with SHA-256 hash linking and mutation-rejection triggers provides tamper-evident logging; Ed25519 replica-manifest signing enables third-party auditability. Core runtime components are complete; HITL approvals, PII guard, and compliance reporting are in progress.
